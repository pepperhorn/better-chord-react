...
- **Guitar chord diagrams (`@pepperhorn/chordl-guitar`).** A new **Guitar** option on the Display toggle renders fretboard "frames" for a chord via svguitar, backed by `@tombatossals/chords-db`. Each chord's **alternate placements** (open, barre, higher positions) are browsable with an A/B/C toggle that shows the base fret. Based on the pepperhorn/frames project. The guitar view (svguitar + shape library, ~200KB) is lazy-loaded, so it only ships when selected.
